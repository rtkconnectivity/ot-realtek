...
T_SW_RESET_REASON sw_reset_type;
...
void common_main(void)
{
    ...
    {
        DBG_DIRECT("Non-Secure World: common_main in app");
    }
    else
    {
        DBG_DIRECT("Secure World: common_main in app");
    }
#endif
...
//add common system code here before enter user defined main function
    ...
    if (!OTP->ftl_init_in_rom && OTP->use_ftl)
    {
        ...
        FLASH_PRINT_INFO1("[FTL] init ret=%d", ftl_pool_init(flash_nor_get_bank_addr(FLASH_FTL),
                                                             flash_nor_get_bank_size(FLASH_FTL),
                                                             rom_ftl_size));
#else
        extern uint32_t (*ftl_init)(uint32_t u32PageStartAddr, uint8_t pagenum);
        FLASH_PRINT_INFO1("[FTL] init ret=%d", ftl_init(flash_nor_get_bank_addr(FLASH_FTL),
...
void ram_init(void)
{
    ...
    //copy psram ro
#if defined (__ARMCC_VERSION) && (__ARMCC_VERSION >= 6010050)
    extern unsigned int Image$$PARAM_SECTION$$RO$$Base;
    extern unsigned int Load$$PARAM_SECTION$$RO$$Base;
    extern unsigned int Image$$PARAM_SECTION$$RO$$Length;
    section_image_base = (unsigned int *)& Image$$PARAM_SECTION$$RO$$Base;
    section_load_base = (unsigned int *)& Load$$PARAM_SECTION$$RO$$Base;
    section_image_length = (unsigned int)& Image$$PARAM_SECTION$$RO$$Length;
#elif defined ( __GNUC__ )
    ...
    //copy PSRAM rw
#if defined (__ARMCC_VERSION) && (__ARMCC_VERSION >= 6010050)
    extern unsigned int Image$$PARAM_SECTION$$RW$$Base;
    extern unsigned int Load$$PARAM_SECTION$$RW$$Base;
    extern unsigned int Image$$PARAM_SECTION$$RW$$Length;
    section_image_base = (unsigned int *)& Image$$PARAM_SECTION$$RW$$Base;
    section_load_base = (unsigned int *)& Load$$PARAM_SECTION$$RW$$Base;
    section_image_length = (unsigned int)& Image$$PARAM_SECTION$$RW$$Length;
#elif defined ( __GNUC__ )
    ...
    //clear PSRAM zi
#if defined (__ARMCC_VERSION) && (__ARMCC_VERSION >= 6010050)
    extern unsigned int Image$$PARAM_SECTION$$ZI$$Base;
    extern unsigned int Image$$PARAM_SECTION$$ZI$$Length;
    section_image_base = (unsigned int *)& Image$$PARAM_SECTION$$ZI$$Base;
    section_image_length = (unsigned int)& Image$$PARAM_SECTION$$ZI$$Length;
#elif defined ( __GNUC__ )
...
static void AppUpdateVectorTable(void)
{
    ...
    const char *ExtIrq[] =
    {
        ...
        "SDIO0", "ISO7816", "Display", "SPORT0 RX", "SPORT0 TX", "SPORT1 RX", "SPORT1 TX", "SHA256",
        "Pub Key Engine", "Flash SEC", "SegCom_CTL", "CAN", "GMAC", "RTZIP", "Slave_Port_Monitor", "PTA_Mailbox",
...
void print_reset_reason(void)
{
    ...
    else
    {
        ...
        sw_reset_type = aon_0x1ae0.reset_reason ;
        aon_0x1ae0.reset_reason = RESET_REASON_WDT_TIMEOUT;
